...
class RetentionSettings(SingletonModel):
    RETENTION_NONE = None
    RETENTION_DAY = 24
    RETENTION_WEEK = 7 * RETENTION_DAY
    ...
    RETENTION_CHOICES = (
        ...
        (RETENTION_DAY, _('Discard most readings after 24 hours')),
    ...
    data_retention_in_hours = models.IntegerField(
        ...
        help_text=_(
            'By default the application stores all readings taken. As there might be DSMR-readings about every second, '
            'this results in several millions of readings each year. This may cause degraded performance. '
            'For that reason you might want to apply retention to this data. Please note that enabling this feature '
            'will NOT discard ALL readings, as it will PRESERVE the first reading of each hour.'
        )
    )
